---
title: "Update Settings"
description: "Configure the Instagram Profile Scraper settings"
api: "POST https://api.lobstr.io/v1/squids/{squid_hash}"
---

Configure your Instagram Profile Scraper squid settings. Control the maximum number of profiles collected per task and across the entire run.

## Headers

<ParamField header="Authorization" type="string" required>
  Your API authentication token. Value: `Token YOUR_API_KEY`
</ParamField>

<ParamField header="Content-Type" type="string" required>
  Request body format. Value: `application/json`
</ParamField>

## Scraper Parameters

Set these parameters in the `params` object:

| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| **max_results** | integer | unlimited | Maximum number of profiles to collect per task |
| **max_unique_results_per_run** | integer | unlimited | Maximum number of unique results retrieved across all tasks in the entire run |
| **discovery_mode** | boolean | false | Expand from your input profiles: after scraping each profile, the crawler also scrapes its `related_profiles`. Use `hop_depth` and `discovered_from` in results to trace the expansion path |

## Squid Settings

Configure general squid settings:

| Setting | Type | Description |
| --- | --- | --- |
| **name** | string | Display name for your squid configuration |
| **concurrency** | integer | Number of parallel scraping threads (default: 1) |
| **export_unique_results** | boolean | Export only unique results (deduplicated) |
| **to_complete** | boolean | Run until all tasks complete |
| **no_line_breaks** | boolean | Remove line breaks from results |

<Note>
  Leave limits empty or null for unlimited collection.
</Note>

<Tip>
  Use `max_unique_results_per_run` to set a hard cap on total profiles when processing large batches.
</Tip>

## Code Examples

<CodeGroup>
```bash cURL
curl -X POST "https://api.lobstr.io/v1/squids/YOUR_SQUID_HASH" \
  -H "Authorization: Token YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Instagram Profile Scraper (1)",
    "concurrency": 5,
    "export_unique_results": true,
    "to_complete": false,
    "no_line_breaks": true,
    "params": {
      "max_results": 100,
      "max_unique_results_per_run": 1000
    }
  }'
```

```python Python
import requests

API_KEY = "YOUR_API_KEY"
BASE_URL = "https://api.lobstr.io/v1"
headers = {"Authorization": f"Token {API_KEY}"}

squid_id = "YOUR_SQUID_HASH"

# Update squid settings
payload = {
    "name": "Instagram Profile Scraper (1)",
    "concurrency": 5,
    "export_unique_results": True,
    "to_complete": False,
    "no_line_breaks": True,
    "params": {
        "max_results": 100,                # Max profiles per task
        "max_unique_results_per_run": 1000 # Total unique profiles limit
    }
}

response = requests.post(f"{BASE_URL}/squids/{squid_id}",
    headers=headers,
    json=payload
)

print(f"Settings updated: {response.json()['name']}")
```
</CodeGroup>

## Response

```json 201
{
  "name": "Instagram Profile Scraper (1)",
  "concurrency": 5,
  "export_unique_results": true,
  "to_complete": false,
  "no_line_breaks": true,
  "params": {
    "max_results": 100,
    "max_unique_results_per_run": 1000
  }
}
```
