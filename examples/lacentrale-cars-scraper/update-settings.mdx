...
Use `fetch_since` to monitor new inventory. For example, setting `fetch_since: 24h` will stop scraping when it reaches listings older than 24 hours (based on `first_online_date`).
