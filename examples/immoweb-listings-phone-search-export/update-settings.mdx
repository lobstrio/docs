...
<Warning>
  `get_listing_details` is enabled by default and costs 4 additional credits per listing. Disable it if you only need listing summaries.
</Warning>
...
response = requests.post(f"{BASE_URL}/squids/{squid_id}",
    ...
    json={
        ...
        "params": {
            ...
            "max_results": 200,
            "functions": {
                "get_listing_details": True  # 4 credits per listing
            }
        }
    }
)
