...
| **get_profile_details** | boolean | false | Fetch additional profile data: current role description and profile banner URL |
...
| **max_results** | integer | — | Total leads to collect before stopping |
| **max_unique_results_per_run** | integer | — | Maximum unique results across all tasks in the run |
| **start_page** | integer | — | Page number to start scraping from (useful for resuming or skipping already-collected pages) |
| **skip_collected_leads** | boolean | false | Skip leads already present in your results database — useful for incremental runs |
| **split_search** | boolean | false | Split the search into smaller sub-searches to bypass Sales Navigator's 2,500-result cap. Use for broad searches that would otherwise be truncated. |
| **fetch_since** | string | — | Stop scraping when leads older than this threshold are reached. Use a relative duration (`24h`, `7d`) or an absolute date (`YYYY-MM-DD HH:MM:SS`) |
