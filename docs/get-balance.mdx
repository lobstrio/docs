...
<ResponseField name="community_reward_credit" type="integer">
  Bonus credits earned through Community Story Rewards. Example: `500`
</ResponseField>

<ResponseField name="community_reward_credit_expires_at" type="string | null">
  Timestamp when community reward credits expire, or `null` if none. Example: `null`
</ResponseField>
...
<ResponseField name="used_slots" type="integer">
  Runs executing right now, summed by the concurrency each one was given. Squids that exist but are not running count for nothing. Example: `3`
</ResponseField>

<ResponseField name="total_available_slots" type="integer">
  How many the plan allows at once. Example: `20`
</ResponseField>

<ResponseField name="has_unpaid_bill" type="object">
  Outstanding unpaid invoice info. Empty object (`{}`) when there is no unpaid bill — despite the name, this is not a boolean.
</ResponseField>
