...
# Shortcut: client.runs.call(squid=squid.id, callback=on_progress) does the
# start + wait in one call. Add timeout=<seconds> to either to bound the wait.
...
# 8. Clean up — delete the squid (optional: keeping it costs nothing)
...
<Note>
  A `Crawler` has both `account` (bool — needs an account or not) and `account_type` (the account type slug it needs, e.g. `linkedin-sync`, `sales-nav-sync`, or `None` if it needs no account). Two crawlers for the same platform can need different account types, so match a connected account on `account_type`, not on the crawler's name.
</Note>
...
| `client.squids.update(hash, ..., accounts=[...])` | POST | `/v1/squids/{hash}` | Update squid settings, parameters, or the linked account list |
| `client.squids.attach_accounts(hash, accounts, replace=False)` | GET + POST | `/v1/squids/{hash}` | Link one or more connected accounts to a squid without dropping the ones already there |
...
<Note>
  **Linking an account.** `accounts` on `update()` is a direct mirror of the API field, which is **full-replace** — sending it detaches any account not included, and leaving it unset (the default) leaves the squid's accounts untouched. `attach_accounts()` is the safe way to add one: it reads the squid's current accounts first and sends the union, so nothing already attached is dropped. Pass `replace=True` to set the list outright instead, including `[]` to detach everything. Both are available on `LobstrClient` and `AsyncLobstrClient`. The read and the write inside `attach_accounts()` are two separate requests, not atomic — a concurrent attach on the same squid between them can still drop one addition.
</Note>
...
| SDK Method | HTTP | Endpoint | Description |
|---|---|---|---|
| `client.runs.start(squid)` | POST | `/v1/runs` | Start a new run and return the `Run` right away, without waiting |
| `client.runs.call(squid, timeout=None, callback=None)` | POST + poll | `/v1/runs` | Start a run **and wait** for it to finish — returns the finished run. Combines `start()` + `wait()` |
...
| `client.runs.wait(hash, timeout=None, callback=None)` | — | — | Poll until the run completes; optional `callback(RunStats)` for progress. Pass `timeout` (seconds) to bound the wait |
...
<Note>
  **Timeouts.** `wait()` and `call()` wait indefinitely by default (`timeout=None`). Set `timeout` to a number of seconds to give up waiting — a `RunTimeout` (a subclass of the built-in `TimeoutError`) is raised. The run is **not** aborted; it keeps running server-side, so you can re-attach later with `runs.wait(run_id)` or `runs.get(run_id)`.
  ...
  try:
      run = client.runs.call(squid=squid.id, timeout=300)  # give up after 5 min
  except RunTimeout as e:
      print(f"Still running — re-attach later with runs.wait('{e.run_id}')")
  ```
</Note>
...
<Note>
  An `Account` also carries `status`, `resets_in`, and `lock_time` — what a client needs to tell whether the account is healthy and available, or currently locked by a run on another squid, before picking it to attach. There is no attach call on `AccountsResource` itself; link an account to a squid with `client.squids.attach_accounts()` above.
</Note>
