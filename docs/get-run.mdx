...
<ResponseField name="mail_done" type="boolean">
  Whether the email delivery was completed. Example: `false`. Note: this specific endpoint names the field `mail_done`/`mail_time` — [List Runs](/docs/list-runs) and [Start Run](/docs/start-run) return the same data as `email_done`/`email_time` instead.
</ResponseField>

<ResponseField name="mail_time" type="string | null">
...
<Note>
  This endpoint does not return an `is_done` field. Use [Get Run Stats](/docs/get-run-stats) if you need a live `is_done` flag while a run is in progress.
</Note>
...
| **slots_limit_exceeded** | The plan's simultaneous runs were all in use |
