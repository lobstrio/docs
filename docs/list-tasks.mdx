...
<Tip>
  Omit `type` unless you specifically need to isolate one task shape — most squids only ever use one or the other, so filtering by `type` on a URL-only crawler will just return nothing.
</Tip>
