...
# A squid isn't ready to run until you've configured its params at
# least once, even if you're keeping every default value as-is.
requests.post(
    f"https://api.lobstr.io/v1/squids/{squid_id}",
    headers={**headers, "Content-Type": "application/json"},
    json={"params": {}}
)
```
...
<Warning>
  `runs.start()` will fail with `SquidNotReady` if you skip the settings
  update above, even when you don't want to change anything from the
  crawler's defaults — a squid isn't considered ready to run until its
  `params` have been explicitly submitted at least once. See [Update
  Squid](/docs/update-squid).
</Warning>
...
# Configure params (required before starting a run, even with no changes)
requests.post(
    f"https://api.lobstr.io/v1/squids/{squid_id}",
    headers=json_headers,
    json={"params": {}}
...
results = requests.get(
    "https://api.lobstr.io/v1/results",
    headers=headers,
    params={"squid": squid_id, "page_size": 100, "page": 1}
).json()
