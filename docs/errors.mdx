...
| `SlotsLimitExceeded` | Your plan's squid/slot allocation is exhausted (e.g. trying to create another squid with no free slots) — distinct from `ConcurrencyLimitExceeded` below, which is about a run's concurrency setting, not squid count |
