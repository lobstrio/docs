...
The lobstr.io API uses standard HTTP status codes. All error responses share one envelope: an `errors` object with `message` (human-readable), `type` (the error code from the tables below), and `code` (the HTTP status, repeated).

```json
{
  "errors": {
    "message": "A required parameter squid is missing from the route.",
    "type": "ParamsNeeded",
    "code": 400
  }
}
...
| `SlotsLimitExceeded` | Your plan's simultaneous runs are all in use — wait for one to finish, or upgrade. Creating squids is unlimited and never raises this. Distinct from `ConcurrencyLimitExceeded` below, which is about the concurrency you asked a single squid for |
...
When a run ends, the response includes a `done_reason` field (slug) and `done_reason_desc` (human-readable message) explaining why it stopped. These are not HTTP errors — they appear in the run object returned by [Get Run](/docs/get-run).
...
Requests that exceed your plan's rate limit return a `429` response, in the same `errors` envelope as every other error:

```json
{
  "errors": {
    "message": "Request was throttled. Expected available in 2 seconds.",
    "type": "...",
    "code": 429
  }
}
...
def api_request(method, url, **kwargs):
    ...
    if response.status_code == 404:
        message = response.json().get("errors", {}).get("message", "Resource not found")
        raise Exception(f"Not found: {message}")

    if response.status_code == 400:
        message = response.json().get("errors", {}).get("message", "Bad request")
